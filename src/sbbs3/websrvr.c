...
static int sess_sendbuf(http_session_t *session, const char *buf, size_t len, BOOL *failed)
{
	...
	while(sent<len && session->socket!=INVALID_SOCKET) {
		...
		switch(sel) {
			case 1:
				if (session->is_tls) {
					if(!HANDLE_CRYPT_CALL(cryptPushData(session->tls_sess, buf+sent, len-sent, &tls_sent), session)) {
...
void http_logon(http_session_t * session, user_t *usr)
{
	...
	if(session->user.number==0)
		...
		putuserrec(&scfg,session->user.number,U_MODEM,LEN_MODEM,session->is_tls?"HTTPS":"HTTP");
